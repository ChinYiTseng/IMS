...
for m = 1:population_size 
    population_list(m,1:j_num*ma_num) = randperm(j_num*ma_num); % Random permutation of the integers from 1 to j_num*ma_num inclusive.
...
for i = 1:population_size
    ...
    MachineJob = zeros(population_size*2,j_num*ma_num); % 每台機器做的Job順序
    MachineTimeBegin = zeros(population_size*2,j_num*ma_num); % 每台機器做每個Job的開始時間
    MachineTimeEnd = zeros(population_size*2,j_num*ma_num); % 每台機器做每個Job的結束時間
    
    for m = 1:population_size*2 % 解碼
        Gen(m,1:j_num*ma_num) = population_list(m,1:j_num*ma_num); % Job順序
        Ma2 = Ma;
        PT2 = PT;
        for j = 1:j_num*ma_num
            for k = 1:ma_num
                if Ma2(Gen(m,j),k) ~= 0
                    Gen_m(m,j) = Ma2(Gen(m,j),k); % 機台順序
                    Gen_t(m,j) = PT2(Gen(m,j),k); % 所需時間
                    Ma2(Gen(m,j),k) = 0;
                    break;
                end
            end
    ...
    for i = 1:population_size
        if (selection_rand(i) <= qk(1))
            population_list(i, 1:j_num*ma_num) = last_population_list(i, 1:j_num*ma_num);
        else
            for j = 1:(population_size-1)
                if (selection_rand(i) > qk(j) && selection_rand(i) <= qk(j+1))
                    population_list(i, 1:j_num*ma_num) = last_population_list((j+1), 1:j_num*ma_num);
                    break;
                end
            end
        end
